...
class ScanTab(QWidget):
    ...
    def add_paths(self, paths):
        new_paths = [p for p in paths
                     if p[0] not in [item[0] for item in self.selected_paths]
                     and p[0] not in self.excluded_paths]
...
class QuarantineTab(QWidget):
    ...
    def restore_selected(self):
        ...
        if id_ is not None:
            self.quarantine.restore_id(id_)
            self.populate_table()
            QMessageBox.information(self, "✅", self.tr("q_restored", id=id_))

    def delete_selected(self):
        id_ = self.get_selected_id()
        if id_ is not None:
            self.quarantine.remove_id(id_)
            self.populate_table()
            QMessageBox.information(self, "🗑️", self.tr("q_deleted", id=id_))
